...
import { scrapeTypeData } from '../api/index.js';
...
//   console.log('category11:', category);
  router.push({
    name: 'typeList', // 假设路由配置中 /type 页面的命名为 TypePage
    params: {
      category: category
    }
  });
...
watch(() => route.params.category, async (newCategory) => {
  try {
    loading.value = true;
    const res = await scrapeTypeData(newCategory);
    if (res && res.length > 0) {
...
onMounted(async () => {
  ...
  // try {
  //   loading.value = true;
  //   const res = await scrapeTypeData(params);
  //   if (res && res.length > 0) {
  //     tvSeriesDataArr.value = res;
